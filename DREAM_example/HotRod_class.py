...
class HotRod:
    def __init__(self, temp_fn, sensor_fn, 
        ...
        self.Mean, self.Unif, self.Log, self.width, self.pmin, self.pmax = self.get_param_stats(param_fn)
    ...
    def xyzarray(self, sensors, radius_inner, radius_outer):
        ...
        temp_ob = temp_ordered.as_matrix()
        temp_obs = temp_ob[1:,:]
    ...
    def get_param_stats(self, param_fn):

        names, distr, mean, width = [],[],[],[]
        with open(param_fn, 'r') as f:
            ...
            for row in f:
                ...
                if c > 0:
                    ...
                    names.append(data[0])
                    distr.append(data[1]) 
                    mean.append(float(data[2])) 
                    width.append(float(data[3]))
            
        self.par_names = names   
        self.npar = len(names)       
        Mean = np.asarray(mean)
        boolean_log = np.in1d(np.asarray(distr),'log')
        boolean_unif = np.in1d(np.asarray(distr),'uniform')
        Log = np.asarray(boolean_log)
        Unif = np.asarray(boolean_unif)
...
#                print('norm',df_par.iloc[i,'Mean'],df_par.iloc[i,'Width'])
                mn = Mean[i] - (3 * width[i])
                mx = Mean[i] + (3 * width[i])
                pmin.append(mn)
                pmax.append(mx) 

        pmin[1:3] = [-1*np.pi] * 2#[-2*np.pi] * 2   
        pmax[1:3] = [np.pi] * 2 
        ...
        width[1:3] = 2*np.pi                 
#        print(Mean, width)
        return Mean, Unif, Log, width, pmin, pmax
    ...
    def launch(self, cal_variables):       
        ...
        if self.paramChoice == 1:
            ...
            self.k = cal_variables[3]
            self.pc = cal_variables[4]
...
#        print("Objective function value: ",obj)
    ...
    def set_propositions(self, proposed_val):
        ...
        if self.paramChoice == 1:
            ...
            self.k = proposed_val[3]
            self.pc = proposed_val[4] 
...
#        print(" ************ ")
        ...
        for i in range(len(self.calibrated_variables)):
            if self.par_names[i] == 'Q_mag':
                print(self.par_names[i],' = ', 10 ** self.calibrated_variables[i])
            else:
                print(self.par_names[i],' = ', self.calibrated_variables[i])
    ...
    def figures_cal(self, save_figures, path):      
        ...
        for j in range(sets):
            ...
            if save_figures == "Yes":
                figname = path + "fig_%s_%i.png"%(file[:-4], j)
    ...
    def figure_3d(self):
        ...
        time_slider_ax = plt.axes([0.1, (2*pos), 0.8, 0.03], facecolor='lightgoldenrodyellow')
...
if __name__ == '__main__':
    fn_name = "./data/20160927_HR_SturtRiver_S1M2c_mod.csv"
    sensor_fn = "./specs/hotrod_sensor_array_updated.dat"
    param_fn = "./cal_input/param_data_opt2.dat"

    td = HotRod(fn_name, sensor_fn,param_fn, paramChoice = 2, radius_inner = 0.047)
    td.solve()
    save_figures = "No"
    path = "./"
    td.figures_cal(save_figures, path)
    td.figure_3d()
